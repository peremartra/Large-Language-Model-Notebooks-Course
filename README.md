...
I will create two separate notebooks for this task. One will involve models from OpenAI, and the other will utilize open-source models provided by Hugging Face. The results obtained in the two notebooks is very different. The system works much better with the OpenAI models. This difference is mainly due to the fact that we must use small models from Hugging Face that can run on our machine or on Colab.
